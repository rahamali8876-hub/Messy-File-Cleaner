...
static int bridge_callback(
    const cleaner_fs_entry_t *platform_entry,
    void *user_data)
{
    ...
    void *ctx)
{
    ...
    return g_active_adapter->platform->fs_walk(
        path,
        bridge_callback,
        &bridge);
}

static int interface_move(
    const char *src,
    const char *dst)
{
    if (!g_active_adapter || !g_active_adapter->platform)
...
void fs_adapter_build_interface(
    fs_adapter_t *adapter,
    fs_interface_t *out)
{
    if (!adapter || !out)
