...
cleaner_thread *cleaner_thread_create(cleaner_thread_fn fn, void *arg) {
  ...
  pthread_create(&t->thread, NULL, thread_entry, pack);

  return t;
}

void cleaner_thread_join(cleaner_thread *t) { pthread_join(t->thread, NULL); }

void cleaner_thread_destroy(cleaner_thread *t) { free(t); }