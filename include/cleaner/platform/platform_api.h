...
typedef int (*cleaner_fs_callback_t)(const cleaner_fs_entry_t *entry,
                                     void *user_data);
...
typedef struct {
  ...
  int (*file_write)(cleaner_file_t *file, const void *data, size_t size);

  int (*file_flush)(cleaner_file_t *file);

  void (*file_close)(cleaner_file_t *file);
  ...
  /* -------- Threadpool -------- */
  cleaner_threadpool_t *(*threadpool_create)(int threads, int queue_size);

  void (*threadpool_submit)(cleaner_threadpool_t *, void (*)(void *), void *);
